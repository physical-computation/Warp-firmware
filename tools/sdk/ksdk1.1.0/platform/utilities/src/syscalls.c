...
extern int _write_atollic(int fd, uint8_t *str, int len);
extern int _read_atollic(int fd, uint8_t *str, int len);
...
caddr_t _sbrk(int32_t incr)
{
    ...
    extern uint8_t  heap_limit asm ("__HeapLimit");
...
{
    return _read_atollic(file, ptr, len);
}
